...
import { Search, PlusCircle, Download, Trash2, Printer, X, Upload, FileCheck, MoreHorizontal, Save, Filter, UserPlus, Calendar } from 'lucide-react';
...
const PurchaseList: React.FC<PurchaseListProps> = ({ purchases, onDelete, onCreateNew, onUpload, onSaveSupplier }) => {
  ...
  const renderPrintPreview = () => {
      ...
      const currency = printingPurchase.currency || 'AOA';
      return (
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-4 rounded-lg shadow-sm border border-slate-200">
        <div>
          <h1 className="text-xl font-bold text-slate-800">Listagem de Compras</h1>
          <p className="text-xs text-slate-500">Gestão de documentos de fornecedores</p>
        </div>
        <div className="flex gap-2">
